...
include "config.php";
    ...
    if(isset($_POST['submit']))
    {
        ...
        for($i=1;$i<=$cnt;$i++){
            ...
            if($_POST[$permission_str] != 0){
                $sql = "UPDATE outingtable1  SET warden_approval_status = '".$_POST[$permission_str]."', warden_comments = '".$_POST[$comment_str]."' WHERE outing_id = '".$_POST[$outing_id_str]."'";
    ...
    function get_outing_details(){
        ...
        $to = $_POST['to'];
            ...
            $query1 = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s where o.warden_approval_status = 0 and o.from_date between '$from' and '$to'and s.year = '".$year."'"; 
            $result1 = mysqli_query($conn, $query1);
            $row1 = mysqli_fetch_array($result1);
            $query2 = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s  where  o.warden_approval_status = 1 and o.from_date between '$from' and '$to'and s.year = '".$year."'"; 
            $result2 = mysqli_query($conn, $query2);
            $row2 = mysqli_fetch_array($result2);
            $query3 = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s  where o.warden_approval_status = 2 and o.from_date between '$from' and '$to'and s.year = '".$year."'"; 
    ...
    function display(){
        include "config.php";
        $year = get_year($_SESSION['username']);
        $query = "SELECT * FROM outingtable1 o inner join student_details s where s.regno = o.regno and o.faculty_approval_status = 1 and o.warden_approval_status = 0  order by from_date desc"; //You don't need a ; like you do in SQL
        $result = mysqli_query($conn, $query);
    ...
    while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results '
        ...
        $body_str = $body_str."<tr><td>" . htmlspecialchars($count) . "</td><td>" . htmlspecialchars($row['from_date']) . "</td><td>" . htmlspecialchars($row['to_date']) . "</td><td>" . htmlspecialchars($row['place']) . "</td><td>" . htmlspecialchars($row['purpose']) . "</td><td>" . htmlspecialchars(get_due($row['arrived_date'], $row['to_date']). " Days") . "</td><td>
