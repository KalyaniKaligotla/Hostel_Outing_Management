...
include "config.php";
    ...
    if(isset($_POST['submit']))
    {
        ...
        for($i=1;$i<=$cnt;$i++){
            ...
            if($_POST[$permission_str] != 0){
                $sql = "UPDATE outingtable1  SET warden_approval_status = '".$_POST[$permission_str]."', warden_comments = '".$_POST[$comment_str]."', warden_approval_time = '".$time."' WHERE outing_id = '".$_POST[$outing_id_str]."'";
    ...
    function get_outing_details(){
        ...
        $to = $_POST['to'];
            ...
            $query = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s  where o.from_date between '$from' and '$to'"; 
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_array($result);
            $query1 = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s where o.warden_approval_status = 0 and o.from_date between '$from' and '$to'"; 
            $result1 = mysqli_query($conn, $query1);
            $row1 = mysqli_fetch_array($result1);
            $query2 = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s  where  o.warden_approval_status = 1 and o.from_date between '$from' and '$to'"; 
            $result2 = mysqli_query($conn, $query2);
            $row2 = mysqli_fetch_array($result2);
            $query3 = "SELECT count(outing_id) FROM outingtable1 o inner join student_details s  where o.warden_approval_status = 2 and o.from_date between '$from' and '$to'"; 
    ...
    function display_rejected(){
        include "config.php";
        $query = "SELECT * FROM outingtable1 o inner join student_details s where s.regno = o.regno and o.warden_approval_status = 2 order by applied_time desc"; //You don't need a ; like you do in SQL
        $result = mysqli_query($conn, $query);
        ...
        $output_str = "<table class='table table-hover' name = 'outing' id = 'outing'> 
            ...
            <td><b>Faculty Comment</b></td>
            <td><b>Warden Comment</b></td>
    ...
    while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results '
        ...
        <td>".htmlspecialchars($row['faculty_comments'])."</td><td>".htmlspecialchars($row['warden_comments'])."</td></tr>";
        $count++;
    }
    ...
    function display_approved(){
        include "config.php";
        $query = "SELECT * FROM outingtable1 o inner join student_details s where s.regno = o.regno and o.warden_approval_status = 1  order by applied_time desc"; 
        $result = mysqli_query($conn, $query);
    ...
    function display_pending(){
        include "config.php";
        $query = "SELECT * FROM outingtable1 o inner join student_details s where s.regno = o.regno and o.faculty_approval_status = 1 and warden_approval_status = 0 order by applied_time desc "; 
        $result = mysqli_query($conn, $query);
    ...
    while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results '
        ...
        $body_str = $body_str."<tr><td onclick= $('#hidden_row".$count."').toggle();>" . htmlspecialchars($row['regno']) .
         "</td><td>" . htmlspecialchars($row['name']) .
          ...
          "</td><td>" . htmlspecialchars($row['to_date']) . 
          "</td><td>" . htmlspecialchars($row['purpose']) . 
          "</td><td>" . htmlspecialchars($row['to_place']) . 
          "</td><td>" . htmlspecialchars($row['faculty_comments']). 
          "</td><td>"."<input type='hidden' id = 'outingid".$count."' name='outingid".$count."' value='".$row['outing_id']."' >
           <select name = 'permission".$count."' id = 'permission".$count."' id = 'permission".$count."' >
